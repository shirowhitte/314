...
include_once 'searchUserCTRL.php';
include_once 'searchUserProfileCTRL.php';
...
$searchUserProfile = new searchUserProfileControl();
  ...
  <div class="sidebar">
    <div class="logo-details">
      ...
      <ul class="nav-links">
        ...
        <li>
          <a href="searchUserProfilePage.php" class="active">
            <i class='bx bx-street-view'></i>
  ...
  <section class="home-section">
    ...
    <div class="home-content">
        ...
        <div class="recent-sales box" style="height: 100%;width:100%">
          <div class="title" >
            <form method="post" action="searchUserProfilePage.php" style="text-align: center;">
                   <h3>Search User Profile</h3><br>
                    <select style="width: 50%;" name="role" id="role">
                       <?PHP 
                      $conn = mysqli_connect("localhost","root","","csit314");
                       $select= "SELECT rolename FROM role";
                      foreach ($conn->query($select) as $rw)
                      {
                          echo "<option value=$rw[rolename]>$rw[rolename]</option>"; 
                      }

                      ?>
                    </select> <br><br>

                    <input style="background-color: #59788e;font-size: 10px;width:30%"type="submit" name="searchUser" value="Search">
            ...
            if (isset($_POST['searchUser'])) 
            {
              ...
              if($searchUserProfile->onSubmit($role))
              {
                $searchUserProfile->validateFields($role);
              }

              if($searchUserProfile->getUserProfile($role))
              {
                  $searchUserProfile->displayUserProfilePage();
              }
              else
              {
                 $searchUserProfile->displayErrMsg();
              }
